...
users:
{u'uid_1': {u'key': [(u'uid', 1)], u'v': 1}, u'_id_': {u'key': [(u'_id', 1)], u'v': 1}, u'servers_1': {u'key': [(u'servers', 1)], u'v': 1}, u'time_in_game_1': {u'key': [(u'time_in_game', 1)], u'v': 1}}
servers:
...
t = db.servers.group(
{
    key: {"tech.RAM": true, "tech.HDD_type": true}, 
    initial: {sum: 0}, 
    reduce: function(doc, prev) { prev.sum += 1}
});
db.histo.insert(t);
db.histo.find().sort({ tech.RAM: 1 });
